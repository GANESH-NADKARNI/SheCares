...
export default function FoodAnalyser() {
  ...
  const allergyInput = allergies.trim(); // Use state value, not DOM
  
  console.log("Sending allergies:", allergyInput); // Debug log
  
  form.append("food_name", name);
  if (desc) form.append("description", desc);
  if (allergyInput) form.append("allergies", allergyInput);
}
      ...
      const parsed: Analysis = {
        ...
        allergy_warning: report.allergy_warning || "",
contains_allergens: Boolean(report.contains_allergens),
      };
...
<div className="space-y-2">
  <Label htmlFor="allergies" className="text-purple-800">
    Allergies (if any)
  </Label>
  <Input 
    id="allergies" 
  ...
  {!allergies.trim() && (
    <p className="text-xs text-orange-600 flex items-center gap-1">
      ...
      Determining allergies may require professional consultation
    </p>
  )}
</div>
                <div className="space-y-2">
...
{!allergies.trim() && (
  <Button
    ...
    variant="outline"
    className="w-full rounded-xl border-purple-300 text-purple-700 hover:bg-purple-50 transition-all"
  >
    Contact Professional for Allergy Testing
  </Button>
)}
...
<Card
  className={`rounded-2xl shadow-md p-4 ${
    analysis?.contains_allergens 
      ? "bg-gradient-to-r from-red-100 to-orange-100 border-red-500" 
      : analysis?.pregnancy_safe 
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        <h3 className="font-semibold text-purple-700">
          {analysis?.contains_allergens ? "⚠️ ALLERGY ALERT" : "Pregnancy Safe"}
        </h3>
      ...
      <CardContent className="pt-2">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-purple-700 mb-1">Allergy Information</h3>
            <p className="text-sm text-purple-700 whitespace-pre-wrap">{analysis.allergy_warning}</p>
          </div>
        </div>
      </CardContent>
    </Card>
