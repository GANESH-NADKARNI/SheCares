...
type PredictionResult = {
  predicted_cycle_length_days: string;  // Changed from number to string (e.g., "25 to 31")
  next_period_date_range: string;       // Changed to date_range
  ovulation_date_range: string;         // Changed to date_range
  fertile_window: string;               // This stays as a range
  last_period_date: string;             // Added this field
};
...
const backendBase = import.meta.env?.VITE_API_URL_COMBINE || "http://0.0.0.0:8001";
...
<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
  {[
    { 
      label: "Next Period", 
      value: modelResult.next_period_date_range || "--",  // Use the range directly
      icon: "📅", 
      delay: 0.4 
    },
    { 
      label: "Ovulation", 
      value: modelResult.ovulation_date_range || "--",    // Use the range directly
      icon: "🌸", 
      delay: 0.5 
    },
    { 
      label: "Fertile Window", 
      value: modelResult.fertile_window || "--",          // Already a range
      icon: "💫", 
      delay: 0.6 
    }
  ].map((item, idx) => (
