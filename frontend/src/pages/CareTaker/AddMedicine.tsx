...
interface Medicine {
  ...
  timestamp: number;
  // optional suggestions returned from the food suggestions API       port=8000
  foodSuggestions?: {
    before: string[];
    after: string[];
  };
...
const App: React.FC = () => {
    ...
    const loadMedicines = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const withDays = data.map((med: any) => {
            const timesPerDay = med.timesSelected.length;
            const remaining = med.totalTablets - (med.consumedTablets || 0);
            const daysLeft = Math.floor(remaining / (med.tabletsPerDose * timesPerDay));
            return { ...med, remainingDays: daysLeft, id: med._id };
          });
          setMedicines(withDays);
        }
      } catch (error) {
  ...
  //   }, [medicines]);
  // Updated reminder check with dosage log creation
  useEffect(() => {
    const checkReminders = async () => {
      ...
      const now = new Date();
      const token = await user.getIdToken();
      const currMinuteKey = Math.floor(now.getTime() / 60000);

      // 1. Create pending logs when scheduled time hits
      for (const med of medicines) {
        for (const slot of med.timesSelected) {
          const hm = med.customTimes[slot] || DEFAULT_TIMES[slot];
          if (!hm) continue;
          ...
          if (now.getHours() === hh && now.getMinutes() === mm) {
            const notifyKey = `${med.id}_${slot}_${currMinuteKey}`;
            if (notifiedRef.current[notifyKey]) continue;
            ...
            try {
              ...
              await fetch('http://localhost:5000/api/dosage-logs', {
                method: 'POST',
                headers: {
            ...
            setShowNotificationBanner({
              title: med.name,
              body: `Time to take ${med.tabletsPerDose} tablet(s) — ${slot} (${formatTime(hm)})`
            });
            setTimeout(() => setShowNotificationBanner(null), 7000);
      ...
      // 2. Auto-mark overdue logs as missed (2-hour rule)
      try {
        await fetch('http://localhost:5000/api/dosage-logs/bulk-miss', {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}` }
        });
      } catch (error) {
        console.error('Failed to bulk update missed:', error);
      }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 200 * 1024) {
      ...
      try {
        ...
        setUploadedImage(compressed);
        console.log('✅ uploadedImage state set (compressed)');
      } catch (e) {
        console.warn('⚠️ Compression failed, falling back to raw file', e);
        const reader = new FileReader();
        reader.onloadend = () => {
          console.log('✅ Raw file read complete');
          setUploadedImage(reader.result as string);
        };
        reader.readAsDataURL(file);
      }
    } else {
      console.log('📄 Reading small image directly...');
      const reader = new FileReader();
      reader.onloadend = () => {
        console.log('✅ File read complete');
        setUploadedImage(reader.result as string);
        console.log('✅ uploadedImage state set');
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const timesPerDay = savedMedicine.timesSelected.length;
        const totalDays = Math.floor(savedMedicine.totalTablets / (savedMedicine.tabletsPerDose * timesPerDay));

        setMedicines(prev => [{ ...savedMedicine, remainingDays: totalDays, id: savedMedicine._id }, ...prev]);
        ...
        setShowNotificationBanner({ title: 'Saved!', body: `${name} added to database` });
        setTimeout(() => setShowNotificationBanner(null), 3000);
      } else {
